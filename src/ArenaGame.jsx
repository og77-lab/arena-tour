...
function RankSparkline(props) {
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.25)", borderRadius: 10, padding: "8px 12px", marginBottom: 6, border: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 10, color: "#475569", fontWeight: 600 }}>last {pts.length} events</div>
      </div>
...
function BracketView(props) {
  ...
  function matchCard(m, ri, mi) {
    ...
    return (
      <div key={ri + "-" + mi} style={{
        background: isCurrent ? "rgba(250,204,21,0.12)" : isPath ? "rgba(129,140,248,0.08)" : "rgba(255,255,255,0.02)",
        border: "1px solid " + (isCurrent ? "#facc15" : isPath ? "rgba(129,140,248,0.35)" : "rgba(255,255,255,0.06)"),
        borderRadius: 6,
        padding: "4px 6px",
        fontSize: 11,
        minWidth: 110,
        ...
        {[a, b].map(function(p, pi){
          if (!p) return (<div key={pi} style={{ color: "#475569", fontSize: 10 }}>—</div>);
          ...
          return (
            <div key={pi} style={{
              ...
              alignItems: "center",
              padding: "2px 0",
              color: p.isP ? "#facc15" : thisWon ? "#e2e8f0" : (w ? "#64748b" : "#cbd5e1"),
              fontWeight: p.isP ? 800 : thisWon ? 700 : 500,
              borderBottom: pi === 0 ? "1px dashed rgba(255,255,255,0.06)" : "none"
            }}>
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.9)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }} onClick={props.onClose}>
      <div style={{ background: "linear-gradient(135deg,#1e293b,#0f172a)", borderRadius: 14, padding: 14, maxWidth: "95vw", width: "100%", border: "1px solid rgba(255,255,255,0.1)", maxHeight: "90vh", display: "flex", flexDirection: "column" }} onClick={function(e){e.stopPropagation();}}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div>
            <span style={{ fontSize: 15, color: tier.col, fontWeight: 800 }}>{tier.name}</span>
            <span style={{ color: "#475569", margin: "0 6px" }}>•</span>
            <span style={{ fontSize: 13, color: "#94a3b8" }}>Draw of {tier.size}</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 3, fontSize: 11, color: "#64748b", marginBottom: 8 }}>
          <span style={{ padding: "2px 6px", borderRadius: 3, background: "rgba(129,140,248,0.1)", color: "#a5b4fc" }}>Your path</span>
          <span style={{ padding: "2px 6px", borderRadius: 3, background: "rgba(250,204,21,0.12)", color: "#facc15" }}>Current match</span>
        </div>
      ...
      <motion.div key="tourney" {...screenAnim} style={W}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <button onClick={function(){setShowBracket(true)}} style={{background:"rgba(129,140,248,0.1)",border:"1px solid rgba(129,140,248,0.3)",color:"#a5b4fc",fontSize:12,padding:"3px 8px",borderRadius:4,cursor:"pointer",fontWeight:700,letterSpacing:0.3}}>🌐 Bracket</button>
