...
async function sSet(v) { try { if (!window.storage) return; if (v === null) { if (window.storage.delete) await window.storage.delete(SK); } else { if (window.storage.set) await window.storage.set(SK, JSON.stringify(v)); } } catch(e){} }
async function sGet() { try { if (!window.storage || !window.storage.get) return null; var r = await window.storage.get(SK); return (r && r.value) ? JSON.parse(r.value) : null; } catch(e){ return null; } }
...
export default function Arena() {
  ...
  useEffect(function () {
    ...
    sGet().then(function(d){fin(d);}).catch(function(){fin(null);});
  ...
  function nextSeason() { if (!S) return; save(mkSeason(S.pName, (S.sNum||1)+1, S)); setScr("hub"); }
